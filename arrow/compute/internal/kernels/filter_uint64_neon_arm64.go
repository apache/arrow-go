...
var filterUint64NeonTables = makeFilterUint64NeonTables()

func makeFilterUint64NeonTables() (tables [528]byte) {
	for mask := 0; mask < 16; mask++ {
		for i := 0; i < 32; i++ {
			tables[mask*32+i] = 0x80
		}
		...
		for lane := 0; lane < 4; lane++ {
			...
			vectorOffset := mask*32 + (pos/2)*16 + (pos%2)*8
			for byteInLane := 0; byteInLane < 8; byteInLane++ {
				tables[vectorOffset+byteInLane] = byte(lane*8 + byteInLane)
			}
			pos++
		}
	}
	...
	for mask := 0; mask < 16; mask++ {
		tables[512+mask] = byte(bits.OnesCount8(uint8(mask)))
	}
	return tables
}
...
func filterUint64Neon(values []uint64, output []uint64, filterData []byte, filterOffset, length int64) bool {
	...
	_filter_uint64_neon(
		...
		unsafe.Pointer(&filterUint64NeonTables[0]),
		length,
	)
	return true
}
