...
//
// then "go generate ./..." writes metric_arrow.go beside the struct. Check the
// result in, as you would with stringer or easyjson, and regenerate when the
// struct changes.
package main
...
// typeList collects -type values, so both "-type A,B" and "-type A -type B" work.
