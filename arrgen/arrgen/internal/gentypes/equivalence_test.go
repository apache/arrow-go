...
// arreflectStructFieldGaps names the Row columns arreflect cannot encode as a
// struct field, and the Go type of each.
//
// Its inferArrowType dispatches on reflect.Kind before checking the types it
// special-cases, so a Go struct that Arrow models as a scalar reaches
// inferStructType, which finds only unexported fields and returns an empty
// struct<>. A tag naming an explicit Arrow type rescues the field, so only the
// untagged spellings are listed. These columns are compared by value in
// TestGapColumnValues; see also README.md.
...
// so its later batches encode the same values against a longer dictionary.
...
// fields and empties its variable-width ones, so nulls are covered too.
func makeRows(n int) []gentypes.Row {
	...
	for i := range rows {
		...
		if null {
			// Leave every pointer nil, and null the []byte columns, which map to
			// null without a pointer.
			r.Bin = nil
			r.LargeBin = nil
			r.ViewBin = nil
			r.DictBin = nil
...
// TestRowSchemaMatchesArreflect checks that the generated schema is what
// arreflect infers, column for column, including names, types and nullability.
...
// TestRowRecordMatchesArreflect checks that for the same input the generated
// encoder and the reflection encoder produce equal columns.
//
// It compares column by column rather than with array.RecordEqual so the
// columns arreflect cannot encode as struct fields can be skipped by name; see
// arreflectStructFieldGaps.
...
// arreflectStructFieldGaps works around: if arreflect learns to encode these
// struct fields, this fails and the skip list should shrink to match.
...
// TestGapColumnValues covers the columns TestRowRecordMatchesArreflect skips.
// There is no reflection-built batch to compare against, so the expected Arrow
// values are spelled out directly.
...
// TestAppendMatchesAppendSlice checks that feeding rows one at a time lands in
// the same place as handing over a slice.
...
// TestAppenderReusableAcrossBatches checks that NewRecordBatch hands over the
// rows so far and leaves the appender ready for the next batch.
...
// TestNilAllocatorUsesDefault covers the constructor's nil-allocator contract.
