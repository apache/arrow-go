...
// formatted source of their Arrow appenders. It does not write to disk.
//
// Output depends on nothing but the package's source, so regenerating an
// unchanged package reproduces the file byte for byte.
...
// errors the type checker reported. Those errors are returned rather than
// raised: the package a generator runs in usually does not compile yet, since
// the call sites are written before the generated file exists.
...
// withLoadContext appends the package's type errors to err as a likely cause.
...
// differing only in the case of their first letter collide this way.
...
// rather than promoted, since arreflect resolves promoted names by breadth and
// tag.
...
// null guards arreflect applies: a nil pointer is null, and so is a nil []byte
// even when the column itself is not nullable.
func renderAppend(idx int, c column) string {
	...
	// sheds its braces so the guard's else does not nest two sets.
