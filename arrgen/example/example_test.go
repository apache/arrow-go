...
// Example encodes a slice of rows in one call. It is the drop-in replacement
// for arreflect.RecordFromSlice[Metric](metrics, mem).
func Example() {
	cpuLoad := 0.75
...
// size boundary, which is what an ingest loop actually looks like. The appender
// reads each row synchronously and never keeps a reference, so the same
// variable is reused for every row and the loop allocates nothing.
...
// It is a package-level variable, so asking for it costs nothing.
