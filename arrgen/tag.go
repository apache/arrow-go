...
// tagOpts is the parsed form of one `arrow:"..."` struct tag.
//
// It is deliberately a mirror of the unexported tagOpts in arreflect: the two
// packages have to agree about what a tag means or generated code would encode
// a different schema than the reflection path, which the equivalence tests
// would then catch. Keep the two in sync when either grows an option.
type tagOpts struct {
	Name             string
	Skip             bool
	Dict             bool
...
// records a diagnostic and surfaces it later, an unusable tag is an error here:
// the generator has a file to write and no reason to defer the complaint.
...
// splitTagTokens splits the option list on commas that are not nested inside
// parentheses, so decimal(18,2) survives as a single token.
...
// validate rejects option combinations that arreflect also rejects, so a struct
// arrgen accepts is always one arreflect can encode.
